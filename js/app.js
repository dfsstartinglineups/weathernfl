...
function createGameCard(gameId, game, isSingleTeam) {
    ...
    if (isDome) {
        ...
        // Explicitly slice the hourly list down to exactly 5 hours (1 before kickoff, next 4 after)
        const hourlySlice = w.hourly.slice(0, 5);
        
        const hoursMarkup = hourlySlice.map((h) => {
            // Safely parse the strict ISO string timestamp generated by the backend
